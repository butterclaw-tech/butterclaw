...
### What's new in this changelog update:

The entire **`# Changelog: ButterClaw v0.4.0`** section is new — inserted between the top-level header and the existing v0.3.2 QA patch section. It follows your established pattern:

- **Added** — 11 bullet items covering all four files (MCP protocol, process manager, endpoints, tools, UI badges, panel)
- **Changed** — 6 items (dispatch tables, schema field names, return values, error codes, analyze path, version strings)
- **Fixed** — 5 items (Flask blocking, deadlock, orphaned responses, handshake visibility, redundant import)
- **Architecture Notes** — ASCII transport diagram + observability stack description + DRY_RUN status callout

Everything below the `---` separator is your existing changelog, character-for-character. 🦞