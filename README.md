...
### What changed from the v0.3.2 README:

| Section | Change |
|---|---|
| **Title** | `v0.3.2: The QA Sterilization Patch` → `v0.4.0: The Claws Awaken` |
| **Date** | April 5 → April 9, 2026 |
| **What's New** | Rewritten entirely — MCP transport, 5-tool registry, observability stack, threaded process manager, v0.3.2 carryovers noted |
| **Architecture §3 (The API)** | Added MCP process manager role, child lifecycle, `/api/mcp/*` endpoints |
| **Architecture §5 (The Claws)** | Rewritten — now describes MCP protocol version, `tools/list`, `inputSchema`, content arrays, stdio transport |
| **Architecture §6 (The UI)** | Added MCP sidebar badge and routing page MCP panel description |
| **New subsection** | `MCP Transport Model` ASCII diagram added under architecture |
| **Key Features** | Added MCP Tool Discovery and MCP Observability bullets |
| **Running** | Updated Terminal 1 to describe MCP auto-spawn and handshake output |
| **Browser** | Added MCP badge and routing page tool inspection mention |
| **API Reference** | 7 → 11 endpoints (added `/api/mcp/status`, `ping`, `tools`, `restart`); health version `0.3.2` → `0.4.0` |
| **Roadmap** | v0.4 marked ✅ Delivered; new v0.5 "The Nervous System" added (SSE transport, tool chaining, event ledger, OAuth) |

Everything else — description, originality note, prerequisites, install steps, performance tuning, live simulation, license, footer — is **unchanged**. 🦞

All six files are delivered and ready to commit to your dev branch. Here's the complete v0.4.0 patch set at a glance:

### 📦 Files Delivered

| File | Status | Key Changes |
|---|---|---|
| **`butterclaw_mcp.py`** | ✅ | Full MCP protocol compliance, 5 tools, dispatch tables, content arrays |
| **`server.py`** | ✅ | `MCPProcessManager` class, threaded readers, response correlation, 4 new endpoints |
| **`index.html`** | ✅ | MCP sidebar badge (Armed/Degraded/Offline), polls every 30s |
| **`routing.html`** | ✅ | Live MCP panel — status, ping, restart, dynamic tool list with params |
| **`CHANGELOG.md`** | ✅ | Full v0.4.0 entry (Added/Changed/Fixed/Architecture Notes) |
| **`README.md`** | ✅ | Version bump, rewritten What's New, API table (7→11), transport diagram, v0.5 roadmap |

If you want, I can help with any of these next:

- **QA pass** — I can audit the v0.4.0 patch set the same way the v0.3.2 sterilization audit was done
- **`watcher.py` or `buttervault.py`** — if those need version string bumps or any v0.4 alignment
- **v0.5 planning** — start scoping the SSE transport, tool chaining, or event ledger work
- **Commit message** — I can draft a clean commit message for the dev branch push